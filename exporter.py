...
api_version_gauge = Gauge('api_version_info', 'API version of the RPC Node', ['version'])
next_upgrade_gauge = Gauge('next_upgrade_version', 'Next upgrade version of the RPC Node', ['version'])
...
def find_config_path():
    """Searches for the rpc-node process and extracts the configuration path from its command line."""
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        if proc.info['name'] == 'rpc-node' and len(proc.info['cmdline']) > 1:
            for arg in proc.info['cmdline']:
                if arg.endswith('.toml'):
                    logging.info(f"Found configuration file at: {arg}")
                    return arg
    logging.warning("rpc-node process not found or config file argument missing.")
    return None
